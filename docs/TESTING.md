...
> 設定 `brainsync.focusDoNotDisturb` を `true` にしてから確認してください。

- [ ] **前提検証（A1）**: `Developer: Reload Window` でウィンドウを再読み込みすると、VS Code の Do Not Disturb が OFF にリセットされる（ステータスバー右のベルアイコンが通常表示に戻る）
- [ ] 作業開始 → ステータスバー右のベルアイコンが bell-slash（DND ON）になる
- [ ] 作業中に他拡張の info 通知トーストが表示されない（通知センターには溜まる）
- [ ] 作業完了 → 本拡張の「休憩する」通知が**表示される**（＝通知の前に DND が解除されている）
- [ ] 一時停止すると通知抑制が解除され、再開すると再び抑制される
- [ ] リセット／休憩開始で通知抑制が解除される
- [ ] ウィンドウ再読み込み後に通知抑制が残らない
